...
export type ApiHelpers = {
  ...
  onDelete(path: string, body: unknown, status?: number, repeat?: number): void;
  ...
  getAbortedCalls(): { method: HttpMethod; path: string; timestamp: number }[];
};
  ...
  return definePlugin<"api", ApiHelpers>("api", {
    ...
    setup() {
      ...
      function installHang(method: HttpMethod, rawPath: string) {
        ...
        log(`installHang completed, mockRoutes.length=${mockRoutes.length}`);
      }
          ...
          try {
            ...
            await Promise.race([
              new Promise<void>((resolve) => {
                callEmitter.addEventListener(eventName, () => resolve(), {
                  once: true,
                });
              }),
        ...
        for (let i = mockRoutes.length - 1; i >= 0; i -= 1) {
          ...
          if (route.isAbsolute) {
            if (route.rawPath === fullUrl) {
              return { route, stubPath: new URL(fullUrl).pathname, mount: "" };
            }
            continue;
          }
          ...
          if (actual.endsWith(stubPath)) {
            ...
            const mount = actual.slice(0, actual.length - stubPath.length);
      ...
      class FakeXHR {
        ...
        private matched?: { route: Route; stubPath: string; mount: string };
        ...
        send(body?: unknown) {
          ...
          const match = matchRoute(this.method as HttpMethod, this.url);
          if (!match) {
          ...
          recordCall({
            ...
            query: (() => {
              ...
              new URL(this.url, window.location.origin).searchParams.forEach(
                (value, key) => {
                  if (!queryParams[key]) {
        ...
        abort() {
          ...
          const stubPath = this.matched?.stubPath || new URL(this.url).pathname;
          log(
      ...
      window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        recordCall({
          ...
          query: (() => {
            const queryParams: Record<string, string | string[]> = {};
            new URL(url, window.location.origin).searchParams.forEach(
              (value, key) => {
                if (!queryParams[key]) {
                  queryParams[key] = value;
                } else if (Array.isArray(queryParams[key])) {
        ...
        log(`Mock found for ${method} ${url} -> ${match.route.rawPath}`);
        ...
        return new Promise<Response>((resolve, reject) => {
          ...
          const onAbort = () => {
            ...
            log(`[API Plugin] Recording fetch abort with path: ${abortPath}`);
            recordAbort({ method, path: abortPath, timestamp: Date.now() });
            log("[API Plugin] Current abortedCalls:", abortedCalls);
            endRequest(requestId);
      ...
      const getCalls = (method?: HttpMethod, path?: string) =>
        calls.filter(
          (call) =>
            ...
            (!path || call.path === (path.startsWith("/") ? path : `/${path}`))
        );
      ...
      const expectNoPending = async () => {
        await waitForIdle();
        const pending = mockRoutes.filter((route) => route.remainingUses > 0);
        if (pending.length) {
          throw new Error(
